...
def run():
    ...
    cur.execute("select video_id, video_url from data_vue_download where is_download=False;")
    task_cfgs = cur.fetchall()
    for task_cfg in task_cfgs:
        video_id = task_cfg[0]
        url = task_cfg[1]
        cur.execute("update data_vue_download set redis_task_id=%s where video_id=%s;", (time_now, video_id))
        conn.commit()
